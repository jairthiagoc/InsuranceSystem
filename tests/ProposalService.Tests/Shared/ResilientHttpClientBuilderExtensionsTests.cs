...
public class ResilientHttpClientBuilderExtensionsTests
{
    ...
    [Fact]
    public void AddResilientHttpClient_WithMinimumValues_ShouldRegisterHttpClient()
    {
        // Arrange
        var services = new ServiceCollection();
        var clientName = "MinimumValuesClient";

        // Act
        var httpClientBuilder = services.AddResilientHttpClient(clientName, options =>
        {
            options.MaxRetryAttempts = 1;
            options.RetryDelayMilliseconds = 1;
            options.CircuitBreakerThreshold = 1;
            options.CircuitBreakerDurationMilliseconds = 1;
